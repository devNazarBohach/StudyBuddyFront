  ...
  async function openProfileCard() {
    ...
    try {
      ...
      if (json.success && json.data) {
        ...
        setSubjects(
          Array.isArray(json.data.subjects) ? json.data.subjects.join(", ") : ""
        );
      } else {
      ...
      <View style={{ flex: 1, backgroundColor: theme.background }}>
        <ScrollView contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={[s.section, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <SettingRow icon="moon-outline" title="Dark mode" value={isDarkMode} onChange={handleDarkMode} theme={theme} fs={fs} />
            <View style={[s.divider, { backgroundColor: theme.border }]} />
            <SettingRow icon="contrast-outline" title="High contrast" value={isHighContrast} onChange={handleHighContrast} theme={theme} fs={fs} />
            <View style={[s.divider, { backgroundColor: theme.border }]} />
            <SettingRow icon="location-outline" title="Share location" value={shareLocation} onChange={handleShareLocation} theme={theme} fs={fs} />
          </View>
          ...
          <View style={[s.section, { backgroundColor: theme.card, borderColor: theme.border }]}>
              ...
              {FONT_SCALE_STEPS.map((step) => {
                ...
                return (
                  ...
                  >
                    <ThemedText
                      style={{
                        fontSize: fs(12 + FONT_SCALE_STEPS.indexOf(step) * 3),
                        fontWeight: "700",
            ...
            {/* Preview */}
            <View style={[s.fontPreview, { backgroundColor: theme.surface, borderColor: theme.border }]}>
              <ThemedText style={{ color: theme.secondaryText, fontSize: fs(12), marginBottom: 4 }}>
                Preview
              </ThemedText>
              <ThemedText style={{ color: theme.text, fontSize: fs(16) }}>
                Regular text looks like this
              </ThemedText>
              <ThemedText style={{ color: theme.text, fontSize: fs(22), fontWeight: "700", marginTop: 4 }}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {role === "STUDENT" && (
                  <>
                    ...
                    <ThemedText style={[s.fieldLabel, { color: theme.secondaryText }]}>
                      Subjects{" "}
                      <ThemedText style={{ color: theme.placeholder, fontSize: fs(12) }}>
                        (comma-separated)
                      </ThemedText>
                    </ThemedText>
                    <TextInput
                ...
                {editMode && (
                  <View style={s.editActions}>
                    ...
                    >
                      <ThemedText style={{ color: theme.text, fontWeight: "600", fontSize: fs(15) }}>Cancel</ThemedText>
                    </Pressable>
                    <Pressable
                    ...
                    >
                      {saving
                        ...
                        : <ThemedText style={{ color: theme.onPrimary, fontWeight: "700", fontSize: fs(15) }}>Save</ThemedText>
                      }
                    </Pressable>
                  </View>
                )}
...
// ── SettingRow ────────────────────────────────────────────────────────────────
function SettingRow({
  icon, title, value, onChange, theme, fs,
}: {
  ...
  return (
    <View style={sr.row}>
      <View style={sr.left}>
        ...
        <ThemedText style={[sr.title, { color: theme.text, fontSize: fs(17) }]}>{title}</ThemedText>
      </View>
      <View style={sr.right}>
        <ThemedText style={[sr.state, { color: theme.secondaryText, fontSize: fs(14) }]}>
          {value ? "On" : "Off"}
        </ThemedText>
        <Switch
          value={value}
...
function makeStyles(theme: AppTheme, fs: (n: number) => number) {
  return StyleSheet.create({
    ...
    title: { textAlign: "center", marginTop: 8, marginBottom: 28, fontWeight: "700", color: theme.text },
    profileCard: { borderRadius: 22, padding: 18, flexDirection: "row", alignItems: "center", marginBottom: 24, borderWidth: 1, elevation: 3 },
    ...
    profileInfo: { flex: 1 },
    profileName: { fontSize: fs(22), fontWeight: "700" },
    profileRole: { marginTop: 4, fontSize: fs(14) },
    editBtn: { width: 38, height: 38, borderRadius: 19, alignItems: "center", justifyContent: "center" },
    section: { borderRadius: 22, padding: 8, marginBottom: 20, borderWidth: 1, elevation: 2 },
    divider: { height: 1, marginHorizontal: 12 },
    // Font size controls
    fontSizeHeader: { flexDirection: "row", alignItems: "center", gap: 12, paddingHorizontal: 12, paddingTop: 12, paddingBottom: 8 },
    fontIconWrap: { width: 40, height: 40, borderRadius: 20, alignItems: "center", justifyContent: "center" },
    fontSizeTitle: { fontSize: fs(17), fontWeight: "600" },
    fontStepsRow: { flexDirection: "row", gap: 10, paddingHorizontal: 12, paddingBottom: 12 },
    fontStep: { flex: 1, height: 52, borderRadius: 14, borderWidth: 1, alignItems: "center", justifyContent: "center" },
    fontPreview: { marginHorizontal: 12, marginBottom: 12, borderRadius: 14, borderWidth: 1, padding: 14 },
    // Actions
    ...
    actionText: { fontSize: fs(17), fontWeight: "600" },
    logoutButton: { marginTop: 6 },
    logoutText: { fontSize: fs(17), fontWeight: "700" },
    // Modal
    ...
    modalTitle: { fontSize: fs(22), fontWeight: "700" },
    ...
    modalName: { fontSize: fs(20), fontWeight: "700" },
    modalEmail: { fontSize: fs(13), marginTop: 2 },
    ...
    fieldLabel: { fontSize: fs(13), fontWeight: "600", marginBottom: 6, marginTop: 4 },
    input: { height: 48, borderWidth: 1, borderRadius: 12, paddingHorizontal: 14, marginBottom: 12 },
