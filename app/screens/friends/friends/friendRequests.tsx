...
export const friendsApi = {
  ...
  async outgoing() {
    const data = await this.getOutgoingRequests();
    return { ok: true, status: 200, data };
  },

  async accept(requester_username: string) {
    const result = await this.acceptRequest(requester_username);
    return { ok: true, status: 200, data: result };
  },

  async reject(requester_username: string) {
    const result = await this.rejectRequest(requester_username);
    return { ok: true, status: 200, data: result };
  },
};
